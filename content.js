...
function setupTimestampUI() {
    ...
    if (!timestampContainer) {
        ...
        title.innerText = "Custom Timestamps by TubeManager";
        ...
        saveButton.addEventListener("click", () => {
            ...
            if (hours == 0 && minutes == 0 && seconds == 0) {
                alert("TubeManager :  No time selected");
                return;
            }
            if (heading === "") {
                alert("TubeManager : Please add some heading")
                return;
            }
...
function saveTimestamp(time, color, heading = "", note = "", links = []) {
    try {
        ...
        let index = timestamps.findIndex((ts) => formatTime(ts.time) === formatTime(time));
        if (index !== -1) {
            alert("TubeManager: Time Stamp for this time already exixts");
            return;
        }
...
async function addTimestampsToList() {
    try {
        ...
        timestamps.forEach(({ time, color, heading, note, links }, index) => {
            ...
            // Use raw seconds so deletion logic and marker removal work reliably
            deleteButton.addEventListener("click", () => deleteTimestamp(time));
...
function deleteTimestamp(timeInSeconds) {
    ...
    try {
        ...
        // Match based on raw numeric seconds
        const index = timestamps.findIndex(ts => Number(ts.time) === Number(timeInSeconds));
        console.log(index, timeInSeconds);

        if (index !== -1) {
            console.log("we are inside")
            timestamps.splice(index, 1);
            localStorage.setItem(videoId, JSON.stringify(timestamps));
            removeTimestampMarker(Number(timeInSeconds));
            addTimestampsToList();
        }
    } catch (error) {
